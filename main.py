...
def momentum_strategy(ticker, start="2018-01-01", end="2025-01-01", target_vol=0.3, cost_rate=0.001, slippage_rate=0.0005, plot=True):
    ...
    data["hold"] = (1 + data["returns"]).cumprod()
    spy["returns"] = spy["Close"].pct_change()
    spy_cum = (1 + spy["returns"]).cumprod()

    combined = pd.DataFrame({
        "Strategy": (1 + data["strategy_net"]).cumprod(),
        f"{ticker} Buy-and-Hold": data["hold"],
    ...
    perf_summary = performance_summary(combined["Strategy"])

    if plot:
        combined.plot(figsize=(12, 6), logy=True)
        plt.title(f"{ticker} Momentum Strategy vs Buy and Hold vs SPY")
        plt.ylabel("Cumulative Log Returns")
        plt.legend()
        plt.show()

    return combined, perf_summary


combined, summary = momentum_strategy("AAPL")
print(summary)
